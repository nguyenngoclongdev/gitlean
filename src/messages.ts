...
export class Messages {
	...
	static async showDebugLoggingWarningMessage(): Promise<boolean> {
		...
		const result = await Messages.showMessage(
			'warn',
			'GitLean debug logging is currently enabled. Unless you are reporting an issue, it is recommended to be disabled. Would you like to disable it?',
	...
	static showGitDisabledErrorMessage() {
		return Messages.showMessage(
			'error',
			'GitLean requires Git to be enabled. Please re-enable Git \u2014 set `git.enabled` to true and reload.',
	...
	static showGitInvalidConfigErrorMessage() {
		return Messages.showMessage(
			'error',
			'GitLean is unable to use Git. Your Git configuration seems to be invalid. Please resolve any issues with your Git configuration and reload.',
		);
	}
	...
	static showGitMissingErrorMessage() {
		return Messages.showMessage(
			'error',
			"GitLean was unable to find Git. Please make sure Git is installed. Also ensure that Git is either in the PATH, or that 'git.path' is pointed to its installed location.",
	...
	static showGitVersionUnsupportedErrorMessage(version: string, required: string): Promise<MessageItem | undefined> {
		return Messages.showMessage(
			'error',
			`GitLean requires a newer version of Git (>= ${required}) than is currently installed (${version}). Please install a more recent version of Git.`,
	...
	static showInsidersErrorMessage() {
		return Messages.showMessage(
			'error',
			'GitLean (Insiders) cannot be used while GitLean is also enabled. Please ensure that only one version is enabled.',
			SuppressedMessages.GitDisabledWarning,
		);
	}
	...
	static async showWhatsNewMessage(version: string) {
		...
		const result = await Messages.showMessage(
			'info',
			`GitLean has been updated to v${version} — check out what's new!`,
			undefined,
			null,
			whatsnew,
		);
		...
		if (result === whatsnew) {
			await env.openExternal(Uri.parse('https://gitlean.amod.io/#whats-new'));
		}
	}
