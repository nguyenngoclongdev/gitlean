...
export class RebaseEditorProvider implements CustomTextEditorProvider, Disposable {
	...
	constructor() {
		this._disposable = Disposable.from(
			window.registerCustomEditorProvider('gitlean.rebase', this, {
	...
	get enabled(): boolean {
		...
		if (Array.isArray(associations)) {
			...
			return association != null ? association.viewType === 'gitlean.rebase' : true;
		}

		const association = associations['git-rebase-todo'];
		return association != null ? association === 'gitlean.rebase' : true;
	}
	...
	async setEnabled(enabled: boolean): Promise<void> {
		...
		if (associations == null) {
			...
			associations['git-rebase-todo'] = enabled ? 'gitlean.rebase' : 'default';
		}
