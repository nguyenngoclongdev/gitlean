...
import type { CreatePullRequestActionContext, GitLensApi, OpenPullRequestActionContext } from '../src/api/gitlean';
...
export async function activate(context: ExtensionContext): Promise<GitLensApi | undefined> {
	...
	if (context.extension.id === 'nguyenngoclongdev.gitlean-insiders') {
		// Ensure that stable isn't also installed
		const stable = extensions.getExtension('nguyenngoclongdev.gitlean');
		if (stable != null) {
			Logger.log('GitLean (Insiders) was NOT activated because GitLean is also enabled');
	...
	if (Logger.willLog('debug')) {
		Logger.debug(
			`GitLean (v${gitlensVersion}): syncedVersion=${syncedVersion}, localVersion=${localVersion}, previousVersion=${previousVersion}, ${
	...
	if (!enabled) {
		Logger.log(`GitLean (v${gitlensVersion}) was NOT activated -- "git.enabled": false`);
	...
	try {
		...
		Logger.error(ex, `GitLean (v${gitlensVersion}) activate`);
		void setEnabled(false);
	...
	Logger.log(
		`GitLean (v${gitlensVersion}${cfg.mode.active ? `, mode: ${cfg.mode.active}` : ''}) activated ${
			GlyphChars.Dot
...
async function showWelcomeOrWhatsNew(context: ExtensionContext, version: string, previousVersion: string | undefined) {
	if (previousVersion == null) {
		Logger.log(`GitLean first-time install; window.focused=${window.state.focused}`);
	...
	if (previousVersion !== version) {
		Logger.log(`GitLean upgraded from v${previousVersion} to v${version}; window.focused=${window.state.focused}`);
	}
