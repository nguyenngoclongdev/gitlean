	...
	private static applyModeConfigurationTransform(e: ConfigurationChangeEvent): ConfigurationChangeEvent {
		if (this._configsAffectedByMode == null) {
			this._configsAffectedByMode = [
				`gitlean.${configuration.name('mode')}`,
				`gitlean.${configuration.name('modes')}`,
				`gitlean.${configuration.name('blame.toggleMode')}`,
				`gitlean.${configuration.name('changes.toggleMode')}`,
				`gitlean.${configuration.name('codeLens')}`,
				`gitlean.${configuration.name('currentLine')}`,
				`gitlean.${configuration.name('heatmap.toggleMode')}`,
				`gitlean.${configuration.name('hovers')}`,
				`gitlean.${configuration.name('statusBar')}`,
			];
		}
